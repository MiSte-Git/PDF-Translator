...
Auth model - three states the UI (MergeSearchDialog) needs to distinguish:
  1. "not configured": no Client-ID/Client-Secret stored yet. The user must
     create an OAuth "Desktop app" client in Google Cloud Console first (this
     app cannot provision a Google Cloud project on its own) - see
     docs/google_drive_setup.md - and paste both values into the dialog's
     Drive panel.
  2. "configured, not connected": Client-ID/Secret are present but there is
     no stored refresh token yet, or the user disconnected.
...
def is_configured() -> bool:
    ...
    return has_api_key("google_drive_client_id", ("GOOGLE_DRIVE_CLIENT_ID",)) and has_api_key(
...
def connect_interactively() -> None:
    ...
    screen - callers run this off the UI thread (see
    ui/workers.py::DriveConnectWorker). Raises DriveAuthError if Client-ID/
    Secret are not configured yet, or if Google did not return a refresh
    token (happens if this exact client already has a live grant for this
    account with no offline-access request pending - vanishingly unlikely
    here since InstalledAppFlow always requests offline access, but guarded
    rather than storing an unusable empty value).
    """
    if not is_configured():
        raise DriveAuthError(
            "Keine Google-Client-ID/Client-Secret hinterlegt. Bitte zuerst im "
            "Drive-Bereich der Ordnersuche speichern."
        )
    from google_auth_oauthlib.flow import InstalledAppFlow  # local import: see module docstring
